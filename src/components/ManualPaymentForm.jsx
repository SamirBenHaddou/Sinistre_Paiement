...
import { isValidIBAN, getIBANErrorMessage } from "@/lib/validation";
const ManualPaymentForm = ({ isOpen, onClose, onSubmit }) => {
  ...
  const handleSubmit = (e) => {
    ...
    // Vérifier la validité de l'IBAN s'il est fourni
    if (formData.iban && !isValidIBAN(formData.iban)) {
      const error = getIBANErrorMessage(formData.iban);
      toast({
        title: "IBAN invalide",
        description: error,
        variant: "destructive",
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input
                ...
                className="w-full bg-slate-900/50 border border-white/20 rounded-lg px-4 py-2 text-white focus:ring-purple-500 focus:border-purple-500"
                required
              />
            </div>
            <div>
              ...
              <input
                ...
                value={formData.iban}
                onChange={handleChange}
                className={`w-full bg-slate-900/50 border rounded-lg px-4 py-2 text-white focus:ring-purple-500 focus:border-purple-500 ${
                ...
                title={
                  formData.iban && !isValidIBAN(formData.iban)
                    ? getIBANErrorMessage(formData.iban)
                    : ""
                }
              />
              {formData.iban && !isValidIBAN(formData.iban) && (
            ...
            <div>
              ...
              <input
                ...
                onChange={handleChange}
                placeholder={formData.beneficiary}
                className="w-full bg-slate-900/50 border border-white/20 rounded-lg px-4 py-2 text-white focus:ring-purple-500 focus:border-purple-500"
              />
            </div>
            <div>
